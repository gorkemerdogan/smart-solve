...
/// @dev All operations are stateless and use `staticcall` to evaluate the target function `f(x)`.
///      Supported schemes: Forward (O(h)), Backward (O(h)), and Centered (O(h²)).
library Differentiation {
    using MathLib for bytes16;

    /// @dev Quadruple precision zero.
    ...
    /// @dev Expects the target function to have signature `function name(bytes16) external pure returns (bytes16)`.
    ///      Uses `abi.decode` to strictly parse the returned 32-byte word.
    ...
    /// @dev Formula: f'(x) ≈ (f(x + h) - f(x - h)) / (2h)
