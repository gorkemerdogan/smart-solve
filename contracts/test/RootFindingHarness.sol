...
contract RootFindingHarness {
    ...
    function qFromFrac(int256 num, int256 den) external pure returns (bytes16 q) {
        require(den != 0, "den=0");
        bytes16 qNum = ABDKMathQuad.fromInt(num);
        bytes16 qDen = ABDKMathQuad.fromInt(den);
        q = qNum.div(qDen);
    }
    ...
    function f_x2_minus_4(bytes16 x) external pure returns (bytes16 y) {
        bytes16 four = ABDKMathQuad.fromInt(4);
        y = x.mul(x).sub(four);
    }
    ...
    function df_2x(bytes16 x) external pure returns (bytes16 y) {
        bytes16 two = ABDKMathQuad.fromInt(2);
        y = two.mul(x);
    }
    ...
    function df_cubic(bytes16 x) external pure returns (bytes16 y) {
        bytes16 three = ABDKMathQuad.fromInt(3);
        bytes16 one   = ABDKMathQuad.fromInt(1);
        bytes16 x2    = x.mul(x);
