...
async function fromQuad(h: PolynomialHarness, hex: string): Promise<string> {
  const raw = await h.toFloat(hex);
  return fmtScaled(raw);
}
      ...
      for (const n of SIZES) {
        ...
        it(`Test ${t}: ${m.label} gas growth with polynomial length n=${n}`, async function () {
          ...
          if (m.method === "evaluateWithDerivative") {
            ...
            outHex = `p=${px}, p'=${dpx}`;
            outDec = `p=${await fromQuad(harness, px)}, p'=${await fromQuad(harness, dpx)}`;
          } else {
            outHex = out as string;
            outDec = await fromQuad(harness, out as string);
          }
