...
describe("MatrixMasterHarness - Gas Growth Tests (Part 2)", function () {
    ...
    // ------------------------------------------------------------
    // Section 1: Gas vs number of nonzero entries (nnz)
    // ------------------------------------------------------------

    describe("Section 1: Gas vs number of nonzero entries (nnz)", function () {
        const NNZ_CASES = [
            { sub: "1.1", bandwidth: 0, label: "diagonal" },
            { sub: "1.2", bandwidth: 1, label: "band-1" },
            { sub: "1.3", bandwidth: 2, label: "band-2" },
            { sub: "1.4", bandwidth: 3, label: "band-3" },
            { sub: "1.5", bandwidth: 4, label: "band-4" },
        ];
    ...
    // ------------------------------------------------------------
    // Section 2: Matrix size vs average gas (transpose, determinant, inversion)
    // ------------------------------------------------------------

    describe("Section 2: Matrix size vs average gas (transpose, determinant, inversion)", function () {
        const SIZE_CASES = [
            { sub: "2.1", n: 2 },
            { sub: "2.2", n: 4 },
            { sub: "2.3", n: 6 },
    ...
    // ------------------------------------------------------------
    // Section 3: Convergence / iteration proxy for power iteration
    // ------------------------------------------------------------

    describe("Section 3: Iteration proxy for power iteration", function () {
        const TOL_CASES = [
            { sub: "3.1", label: "veryLoose", num: 1n, den: 100n },               // 1e-2
            { sub: "3.2", label: "loose", num: 1n, den: 10_000n },                // 1e-4
            { sub: "3.3", label: "medium", num: 1n, den: 1_000_000n },            // 1e-6
            { sub: "3.4", label: "tight", num: 1n, den: 100_000_000n },           // 1e-8
            { sub: "3.5", label: "veryTight", num: 1n, den: 10_000_000_000n },    // 1e-10
        ];
    ...
    // ------------------------------------------------------------
    // Section 4: input class vs determinant
    // ------------------------------------------------------------

    describe("Section 4: input class vs determinant", function () {
        const CLASS_CASES: Array<{
            sub: string;
            cls: "well-conditioned" | "singular" | "near-singular" | "pivoting-required" | "diagonally-dominant";
        }> = [
                { sub: "4.1", cls: "well-conditioned" },
                { sub: "4.2", cls: "singular" },
                { sub: "4.3", cls: "near-singular" },
                { sub: "4.4", cls: "pivoting-required" },
                { sub: "4.5", cls: "diagonally-dominant" },
            ];
    ...
    // ------------------------------------------------------------
    // Section 5: input class vs inverse
    // ------------------------------------------------------------

    describe("Section 5: input class vs inverse", function () {
        const CLASS_CASES: Array<{
            sub: string;
            cls: "well-conditioned" | "singular" | "near-singular" | "pivoting-required" | "diagonally-dominant";
        }> = [
                { sub: "5.1", cls: "well-conditioned" },
                { sub: "5.2", cls: "singular" },
                { sub: "5.3", cls: "near-singular" },
                { sub: "5.4", cls: "pivoting-required" },
                { sub: "5.5", cls: "diagonally-dominant" },
            ];
    ...
    // ------------------------------------------------------------
    // Section 6: matrix pattern vs gas
    // ------------------------------------------------------------

    describe("Section 6: matrix pattern vs gas", function () {
        const PATTERN_CASES = [
            { sub: "6.1", pattern: "identity" as const },
            { sub: "6.2", pattern: "triangular" as const },
            { sub: "6.3", pattern: "dense_random" as const },
            { sub: "6.4", pattern: "sparse_random" as const },
            { sub: "6.5", pattern: "pivoting-required" as const },
        ];
